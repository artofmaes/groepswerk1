...
if (isset($_POST['registerbutton'])) {
    ...
    if(array_filter($errors)) {
        ...
        $sql = "insert into user(user_voornaam, user_naam, user_email, user_username, user_date, user_password) values('$user_voornaam', '$user_naam', '$user_email', '$user_username', '$user_date', '$user_password')";

        ExecuteSQL($sql);

        header('Location: index.php');//-------------------------------------------------------------------------------------------------------------------------------- redirecten naar de juiste site!!!!
    }
...
?>
  ...
  <!DOCTYPE html>
   <html>
    ...
    <body>
        <form id="registration_form" method="post" action="register2.php">    <!-- action nakijken ---------------------------------->
